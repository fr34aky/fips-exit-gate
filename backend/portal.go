...
type portal struct {
	...
	pacExit         string                  // exit SOCKS target for the served PAC ("" = derive from publicURL)
...
func (p *portal) routes(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /fips.pac", p.fipsPAC)
...
func (p *portal) help(w http.ResponseWriter, r *http.Request) {
	...
	if p.pacExitTarget() != "" {
		data["PacURL"] = p.publicURL + "/fips.pac"
	}
	p.render(w, "help.html", data)
}

// pacTemplate is the browser proxy auto-config served at /fips.pac. %s is the
// exit's <npub>.fips:PORT SOCKS target. Mirrors deploy/fips.pac: .fips / fd00::/8
// / localhost go DIRECT, everything else through the exit (no DIRECT fallback, so
// a down exit fails closed rather than leaking around the proxy).
const pacTemplate = `// fips.pac — generated by the fips-exit portal.
...
// pacExitTarget is the SOCKS target the served PAC points at: PORTAL_PAC_EXIT if
// set, else the portal's own <npub>.fips host with the default connectivity port
// (works out of the box when the portal and exit are the same host).
func (p *portal) pacExitTarget() string {
	if p.pacExit != "" {
		return p.pacExit
	}
	if u, err := url.Parse(p.publicURL); err == nil {
		if h := u.Hostname(); h != "" && strings.HasSuffix(h, ".fips") {
			return h + ":1080"
		}
	}
	return ""
...
// fipsPAC serves the browser proxy auto-config. Public (linked from the FAQ).
func (p *portal) fipsPAC(w http.ResponseWriter, r *http.Request) {
	exit := p.pacExitTarget()
	if exit == "" {
		http.Error(w, "PAC not available: set PORTAL_PAC_EXIT (or a <npub>.fips PORTAL_PUBLIC_URL)", http.StatusNotFound)
		return
	}
	w.Header().Set("Content-Type", "application/x-ns-proxy-autoconfig")
	w.Header().Set("Cache-Control", "no-store")
	fmt.Fprintf(w, pacTemplate, template.JSEscapeString(exit))
}
